...
const IrasavingForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      irasavingAddedList.current[recordIndex].documents.push({
    ...
    const saveIrasaving = () => {
        ...
        : [
            {
              _modal: 'SavingList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'irasaving.id': arr.id },
              _data: { $set: { 'irasaving.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...irasavingAddedList.current];
          newlist[recordIndex] = arr;
          irasavingAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'IRA Saving Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          irasavingAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeIrasaving = () => {
      if (irasavingAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            irasavingAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { irasaving: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    label="IRA Account"
                    placeholder="--Select--"
                    options={formList.iraAccount}
                    required="IRA Account is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <GroupInput
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="primaryAccount"
                    label="Are you a Primary Responsible for this HSA account?"
                    values={['Yes', 'No']}
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.irasavingComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.irasavingComments}
                    ...
                    onChange={(e) => {
                      formRef.current.irasavingComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
