...
const InvestmentsaveForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      investmentsaveAddedList.current[recordIndex].documents.push({
    ...
    const saveInvestmentsave = () => {
        ...
        : [
            {
              _modal: 'SavingList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'investmentsave.id': arr.id },
              _data: { $set: { 'investmentsave.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...investmentsaveAddedList.current];
          newlist[recordIndex] = arr;
          investmentsaveAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Investment Saving Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Investment Save Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          investmentsaveAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeInvestmentsave = () => {
      if (investmentsaveAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            investmentsaveAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Investment Save Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _find: { _id: pageData.current._id },
                _data: {
                  $pull: { investmentsave: { id: formRef.current.id } },
                },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this Investment Saveing Details?'
        );
      }
    };
    return (
            ...
            <form>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="investedCompany"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Invested Company"
                    placeholder="Invested Company"
                    required="Invested Company is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <GroupInput
                ...
                <div className="w-1/3 mr-5">
                  <DatePicker
                    ...
                    uiRefresh={ui}
                    label="Invested On"
                    placeholder="Invested On"
                    required="Invested On is required"
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="investedAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Invested Amount/Bond $"
                    placeholder="Invested Amount/Bond $"
                    required="Invested Amount/Bond is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <GroupInput
                    ...
                    uiRefresh={ui}
                    label="Year To Close"
                    placeholder="Year To Close"
                    required="Year To Close is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <DatePicker
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="primaryAccount"
                    label="Do you interested to build own company/product?"
                    values={['Yes', 'No']}
                ...
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="joinAccount"
                    label="Is your product is Inc/LLC/public?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.investmentsaveComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.investmentsaveComments}
                    placeholder="DAFs, gifts of cash, gifts in a will
                    Real estate, Life insurance policies, Capital assets such as land,
                    buildings, macinery,
                    and appreciated closely-held securities that you've owned for atleast one year"
                    onChange={(e) => {
                      formRef.current.investmentsaveComments =
...
export default InvestmentsaveForm;
