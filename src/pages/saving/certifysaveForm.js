...
const CertifysaveForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      certifysaveAddedList.current[recordIndex].documents.push({
    ...
    const saveCertifysave = () => {
        ...
        : [
            {
              _modal: 'SavingList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'certifysave.id': arr.id },
              _data: { $set: { 'certifysave.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...certifysaveAddedList.current];
          newlist[recordIndex] = arr;
          certifysaveAddedList.current = newlist;
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          certifysaveAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeCertifysave = () => {
      if (certifysaveAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            certifysaveAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { certifysave: { id: formRef.current.id } } },
              },
            ];
...
export default CertifysaveForm;
