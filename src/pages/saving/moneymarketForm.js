...
const MoneymarketForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      moneymarketAddedList.current[recordIndex].documents.push({
    ...
    const saveMoneymarket = () => {
        ...
        : [
            {
              _modal: 'SavingList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'moneymarket.id': arr.id },
              _data: { $set: { 'moneymarket.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...moneymarketAddedList.current];
          newlist[recordIndex] = arr;
          moneymarketAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Money-Market Saving Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Money-market Save Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          moneymarketAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeMoneymarket = () => {
      if (moneymarketAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            moneymarketAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Money-market Save Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { moneymarket: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Money Market Plan Type"
                    placeholder="--Select--"
                    options={formList.mmplanType}
                    required="Money Market Plan Type is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <GroupInput
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.moneymarketComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.moneymarketComments}
