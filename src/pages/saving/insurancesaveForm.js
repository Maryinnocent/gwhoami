...
const InsurancesaveForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      insurancesaveAddedList.current[recordIndex].documents.push({
    ...
    const saveInsurancesave = () => {
        ...
        : [
            {
              _modal: 'SavingList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'insurancesave.id': arr.id },
              _data: { $set: { 'insurancesave.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...insurancesaveAddedList.current];
          newlist[recordIndex] = arr;
          insurancesaveAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Insurance Saving Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Insurance Save Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          insurancesaveAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeInsurancesave = () => {
      if (insurancesaveAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            insurancesaveAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Insurance Save Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { insurancesave: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Name of the LIC"
                    placeholder="Name of the LIC"
                    required="Name of the LIC is required"
                    icon={faSunPlantWilt}
                  />
                </div>
                <div className="w-1/3 mr-5">
              ...
              <div className="flex w-full justify-start items-center mt-3">
                ...
                <div className="w-1/3 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="maturityAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Maturity Amount $"
                    placeholder="Maturity Amount $"
                    required="Maturity Amount $ is required"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="quarteramount"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Monthly/Quarterly Amount $"
                    placeholder="Monthly/Quarterly Amount $"
                    required="Monthly/Quarterly Amount $ is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <GroupInput
                ...
                <div className="w-1/3 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Agent address/Phone #"
                    placeholder="Agent address/Phone #"
                    required="Agent address/Phone # is required"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center mt-3">
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="nonfixedAccount"
                    formRef={formRef}
                    ui={ui}
                    name="nonfixedAccount"
                    label="Is the monthly payment is fixed or non-fixed?"
                    values={['Yes', 'No']}
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.insurancesaveComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.insurancesaveComments}
                    placeholder="DAFs, gifts of cash, gifts in a will
                    Real estate, Life insurance policies, Capital assets such as land, buildings,machinery, 
                    and appreciated closely-held securities that you've owned for at least one year"
                    onChange={(e) => {
                      formRef.current.insurancesaveComments =
...
export default InsurancesaveForm;
