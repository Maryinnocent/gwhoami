...
const VehiclespayForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
        vehiclespayAddedList.current[recordIndex].documents.push({
    ...
    const saveVehiclespay = () => {
        ...
        : [
            {
              _modal: 'InsuranceList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'vehiclespay.id': arr.id },
              _data: { $set: { 'vehiclespay.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...vehiclespayAddedList.current];
          newlist[recordIndex] = arr;
          vehiclespayAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Vehiclespay Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Vehiclespay Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          vehiclespayAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeVehiclespay = () => {
      if (vehiclespayAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            vehiclespayAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Vehiclespay Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { vehiclespay: { id: formRef.current.id } } },
              },
            ];
          ...
          <div className="pt-5 pb-3">
            ...
            <div className="w-1/3 mr-5">
               ...
               <div className="w-1/3 mr-5">
                        <InputSelect
                    styleClass="flex flex-col mb-4"
                    formKey="insuranceName"
                    ID="insuranceName"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Insurance Name"
                    placeholder="--Select--"
                    options={formList.insuranceName}
               ...
               <div className="w-1/3 mr-5">
                  ...
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="policyPeriod"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Policy Period"
                    placeholder="Policy Period"
                    required="Policy Period is required"
                    icon={faUser}
                  />
                </div>  
                <div className="w-1/3 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="policyAddress"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Policy Address"
                    placeholder="Policy Address"
                    required="Policy Address is required"
                    icon={faUser}
                  />
                </div>  
                ...
                <div className="w-1/3 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="totalPremium"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Total Premium"
                    placeholder="Total Premium"
                    required="Total Premium is required"
                    icon={faUser}
                  />
                </div>         
                      </div>
