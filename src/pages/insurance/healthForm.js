...
const HinsuranceForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
        hinsuranceAddedList.current[recordIndex].documents.push({
    ...
    const saveHinsurance = () => {
        ...
        : [
            {
              _modal: 'InsuranceList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'hinsurance.id': arr.id },
              _data: { $set: { 'hinsurance.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Hinsurance Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Hinsurance Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeHinsurance = () => {
      if (hinsuranceAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this hinsurance Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { hinsurance: { id: formRef.current.id } } },
              },
            ];
