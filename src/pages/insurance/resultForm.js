...
const ResultForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
        resultAddedList.current[recordIndex].documents.push({
    ...
    const saveResult = () => {
        ...
        : [
            {
              _modal: 'InsuranceList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'result.id': arr.id },
              _data: { $set: { 'result.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Result Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Result Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeResult = () => {
      if (resultAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Result Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { result: { id: formRef.current.id } } },
              },
            ];
