...
const OtherForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, otherAddedList }) => {
    const [ui] = useState(-1);
    ...
    const saveOther = () => {
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Other Deatails added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Other and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeOther = () => {
      if (otherAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Other?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
