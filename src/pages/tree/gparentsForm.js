...
const GparentsForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, gparentsAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      gparentsAddedList.current[recordIndex].documents.push({
    ...
    const saveGparents = () => {
        ...
        : [
            {
              _modal: 'TreeList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'gparents.id': arr.id },
              _data: { $set: { 'gparents.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...gparentsAddedList.current];
          newlist[recordIndex] = arr;
          gparentsAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Grand Parents Deatails added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Grand Parents and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          gparentsAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeGparents = () => {
      if (gparentsAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            gparentsAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Grand Parents?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { gparents: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="gpBirthLocation"
                    formRef={regRef}
                    uiRefresh={ui}
                    label="Birth Location Address"
                    placeholder="Birth Location Address"
                    required="Birth Location Address is required"
                    icon={faAddressCard}
                  />
                </div>
                <div className="w-1/3 mr-5">
                  ...
                  <div className="flex ml-5">
                    <div class="mr-5">
                      <input
                        ...
                        checked={formRef.current.isAlive}
                        onChange={(e) => {
                          formRef.current.isAlive = e.currentTarget.checked;
                          subRefresh(Date.now());
                        }}
                      />
                      <label
              ...
              <div className="flex w-full justify-start items-center mt-3">
                ...
                <div className="w-1/3 mr-5">
                  <GroupInput
                    ...
                    required="Death Place is required"
                    icon={faCity}
                  />
                </div>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="gpDeathlocation"
                    formRef={regRef}
                    uiRefresh={ui}
                    label="Death Location Address"
                    placeholder="Death Location Address"
                    required="Death Location Address is required"
                    icon={faAddressCard}
                  />
                </div>
                <div className="w-1/3 mr-5">
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.gpComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.gpComments}
                    onChange={(e) => {
                      formRef.current.gpComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
