...
const PersonalForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
        personalAddedList.current[recordIndex].documents.push({
    ...
    const savePersonal = () => {
        ...
        : [
            {
              _modal: 'CertificateList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'personal.id': arr.id },
              _data: { $set: { 'personal.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Personal Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Personal Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removePersonal = () => {
      if (personalAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Personal Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { personal: { id: formRef.current.id } } },
              },
            ];
