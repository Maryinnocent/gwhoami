...
const HonorableForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
        honorableAddedList.current[recordIndex].documents.push({
    ...
    const saveHonorable = () => {
      ...
      let params = isNew
        ? [
            {
              _modal: 'CertificateList',
              _condition: 'update',
              _find: { _id: pageData.current._id },
              _data: { $push: { honorable: arr } },
            },
          ]
        : [
            {
              _modal: 'CertificateList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'honorable.id': arr.id },
              _data: { $set: { 'honorable.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Honorable Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Honorable Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeHonorable = () => {
      if (honorableAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Honorable Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { honorable: { id: formRef.current.id } } },
              },
            ];
