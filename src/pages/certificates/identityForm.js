...
const IdentityForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
        identityAddedList.current[recordIndex].documents.push({
    ...
    const saveIdentity = () => {
        ...
        : [
            {
              _modal: 'CertificateList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'identity.id': arr.id },
              _data: { $set: { 'identity.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Identity Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Identity Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeIdentity = () => {
      if (identityAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Identity Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { identity: { id: formRef.current.id } } },
              },
            ];
