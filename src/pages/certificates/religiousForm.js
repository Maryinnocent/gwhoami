...
const ReligiousForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
        religiousAddedList.current[recordIndex].documents.push({
    ...
    const saveReligious = () => {
        ...
        : [
            {
              _modal: 'CertificateList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'religious.id': arr.id },
              _data: { $set: { 'religious.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Religious Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Religious Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeReligious = () => {
      if (religiousAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Religious Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { religious: { id: formRef.current.id } } },
              },
            ];
