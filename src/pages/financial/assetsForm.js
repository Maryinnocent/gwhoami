...
const AssetsForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
        assetsAddedList.current[recordIndex].documents.push({
    ...
    const saveAssets = () => {
        ...
        : [
            {
              _modal: 'FinancialList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'assets.id': arr.id },
              _data: { $set: { 'assets.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Assets Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Assets Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeAssets = () => {
      if (assetsAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Assets Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { assets: { id: formRef.current.id } } },
              },
            ];
          ...
          <div className="pt-5 pb-3">
            ...
            <div className="w-1/3 mr-5">
                  <GroupInput
                    ...
                    required="Cash equivalents $ is required"
                    icon={faUser}
                  />
                </div>
                <div className="w-1/3 mr-5">
                ...
                <div className="w-1/3 mr-5">
                  <GroupInput
                    ...
                    placeholder="Property / Plant / Equipments"
                    required="Property / Plant / Equipments is required"
                    icon={faUser}
                  />
                </div>     
                <div className="w-1/3 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="vehiclesAssets"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Vehicles Assets "
                    placeholder="Vehicles Assets $"
                    required="Vehicles Assets is required"
                    icon={faUser}
                  />
                </div>     
                        </div>
                ...
                <div className="w-1/3 mr-5">
                  <GroupInput
                    ...
                    required="Other / Parents $ is required"
                    icon={faUser}
                  />
                </div>     
