...
const SportsForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, sportsAddedList }) =>
{
    ...
    {
        sportsAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveSports = () =>
    {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'CrimeList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'sports': arr } } }] :
            [{ _modal: 'CrimeList', _condition: 'update', _find: { _id: pageData.current._id, 'sports.id': arr.id }, _data: { $set: { "sports.$": arr } }, _options: { upsert: false } }];
        (async () =>
        {
            ...
            {
                arr.isSubmit = true;
                let newlist = [...sportsAddedList.current];
                newlist[recordIndex] = arr;
                sportsAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Sports added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () =>
    {
        ...
        {
            ToastMessage({ type: 'error', message: 'Save the Sports Crime and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) =>
    {
        ...
        {
            if (res === 'no') return;
            sportsAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeSports = () =>
    {
        if (sportsAddedList.current[recordIndex].saved === false)
        {
            alertRef.current.showConfirm((res) =>
            {
                if (res === 'no') return;
                sportsAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Sports?');
        } else
        {
            alertRef.current.showConfirm((res) =>
            {
                if (res === 'no') return;
                let params = [{ _modal: 'CrimeList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'sports': { id: formRef.current.id } } } }];
                apiPostCall('/api/common/common_mutiple_insert', { _list: params });
                sportsAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Sports Crime?');
        }
    }
    return (
        <>
            ...
            <div className="p-5 border rounded shadow-md relative" ref={currentDom}>
                <i
                    ...
                    onClick={removeSports}
                ></i>
                <div className="pt-5 pb-3">
                    <form>
                    <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5"> 
                                    <label>Incident / Crime</label>
                                    <select
                                        className={`border w-full p-2 rounded ${!formRef.current.sportsIncident? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.sportsIncident} onChange={e => { formRef.current.sportsIncident = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">Select</option>
                                       {formList.sportsIncident.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                                </div>
                            <div className="w-1/3 mr-5">
                                <label>Sports Name</label>
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.sportsName? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.sportsName} onChange={e => { formRef.current.sportsName = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">Select</option>
                                       {formList.sportsName.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                            </div>
                            <div className="w-1/3 mr-5">
                                <label>Game Category</label>
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.gameCategory? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.gameCategory} onChange={e => { formRef.current.gameCategory = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">Select</option>
                                       {formList.gameCategory.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                                </div>
                            </div>   
                            <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <div className="w-1/3 mr-5"> 
                                    ...
                                    <input
                                        type="text"
                                        value={formRef.current.teamName}
                                        placeholder="Second Name"
                                        className={`w-full rounded border ${!formRef.current.teamName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.teamName = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                         </div>  
                         <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <label>Incident Start Date</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.sportsInsStartDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.sportsInsStartDate ? new Date(formRef.current.sportsInsStartDate) : ''}
                                    onChange={date => { formRef.current.sportsInsStartDate = date; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>Incident End Date</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.sportsInsEndDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.sportsInsEndDate ? new Date(formRef.current.sportsInsEndDate) : ''}
                                    onChange={date => { formRef.current.sportsInsEndDate = date; subRefresh(Date.now()); }}
                                />
                                </div>
                                <div className="w-1/3 mr-5"> 
                                    <label>Location</label>
                                            <input
                                                type="text"
                                                value={formRef.current.sportsLocation}
                                                placeholder="Location"
                                                className={`w-full rounded border ${!formRef.current.sportsLocation ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                                onChange={e => { formRef.current.sportsLocation = e.currentTarget.value; subRefresh(Date.now()); }}
                                            />
                                </div>
                        </div> 
                        <div className="flex w-full justify-start items-center mt-3">
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>Is this accidentally or purposfully?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                                        checked={formRef.current.isAccidentally }
                                        onChange={e => { formRef.current.isAccidentally = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                            ...
                            <div className="w-1/3 mr-5">
                                <label>Did you mention for refy?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        ...
                                        checked={formRef.current.isRefy }
                                        onChange={e => { formRef.current.isRefy = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                                ...
                                <div className="w-1/3 mr-5">
                                <label>Is this involved opponent team?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        ...
                                        checked={formRef.current.isOpponent}
                                        onChange={e => { formRef.current.isOpponent = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                       ...
                       <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>Did you get any wild or red card?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        ...
                                        checked={formRef.current.isAnyCard }
                                        onChange={e => { formRef.current.isAnyCard = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                            ...
                            <div className="w-1/3 mr-5">
                            <label>Refry name</label>
                                    <input
                                        type="text"
                                        value={formRef.current.refryName}
                                        placeholder="Case / FIR Number"
                                        className={`w-full rounded border ${!formRef.current.refryName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.refryName = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                            </div>
                            <div className="w-1/3 mr-5">
                                <label>Disposition</label>
                                    <input
                                        type="text"
                                        value={formRef.current.disposition}
                                        placeholder="Penalty Amount"
                                        className={`w-full rounded border ${!formRef.current.disposition ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.disposition = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                            </div>
                            </div>                
                            <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>Opponent Team Name</label>
                                    <input
                                        type="text"
                                        value={formRef.current.opponentTeamName}
                                        placeholder="Opponent Team Name"
                                        className={`w-full rounded border ${!formRef.current.opponentTeamName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.opponentTeamName = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                            </div>
                            <div className="w-1/3 mr-5">
                               <label>Number of Wild Card</label>
                               <input
                                        type="text"
                                        value={formRef.current.numberOfCard}
                                        placeholder="Opponent Team Name"
                                        className={`w-full rounded border ${!formRef.current.numberOfCard ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.numberOfCard = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                                <div className="w-1/3 mr-5"> 
                                    <label>Candidate First Name</label>
                                            <input
                                                type="text"
                                                value={formRef.current.candidateName}
                                                placeholder="Candidate First Name"
                                                className={`w-full rounded border ${!formRef.current.candidateName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                                onChange={e => { formRef.current.candidateName = e.currentTarget.value; subRefresh(Date.now()); }}
                                            />
                                    </div>
                                </div> 
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                ...
                                <textarea
                                    className={`w-full rounded border ${!formRef.current.sportsComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                    value={formRef.current.sportsComments}
                                    placeholder="Game Category: County/School/state/International"
                                    onChange={e => { formRef.current.sportsComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                    rows={4}
                                >
                                </textarea>
                            </div>
