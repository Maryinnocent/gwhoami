...
const PolicecaseForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, policecaseAddedList }) => {
    ...
    const completeHandler = (event) => {
        policecaseAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const savePolicecase = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'CrimeList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'policecase': arr } } }] :
            [{ _modal: 'CrimeList', _condition: 'update', _find: { _id: pageData.current._id, 'policecase.id': arr.id }, _data: { $set: { "policecase.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...policecaseAddedList.current];
                newlist[recordIndex] = arr;
                policecaseAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Policecase Deatails added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Policecase and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            policecaseAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removePolicecase= () => {
        if (policecaseAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                policecaseAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Policecase?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'CrimeList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'policecase': { id: formRef.current.id } } } }];
                ...
                <div className="pt-5 pb-3">
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5"> 
                                    ...
                                    <select
                                        className={`border w-full p-2 rounded ${!formRef.current.caseType? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.caseType} onChange={e => { formRef.current.caseType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">Select</option>
                                       {formList.caseType.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                                </div>
                            <div className="w-1/3 mr-5">
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <label>Start Date</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.startDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.startDate ? new Date(formRef.current.startDate) : ''}
                                    onChange={date => { formRef.current.startDate = date; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>End Date</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.endDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.endDate ? new Date(formRef.current.endDate) : ''}
                                    onChange={date => { formRef.current.endDate = date; subRefresh(Date.now()); }}
                                />
                                </div>
                                <div className="w-1/3 mr-5"> 
                                    <label>Country / Village</label>
                                            <input
                                                type="text"
                                                value={formRef.current.village}
                                                className={`w-full rounded border ${!formRef.current.village ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                                onChange={e => { formRef.current.village = e.currentTarget.value; subRefresh(Date.now()); }}
                                            />
                                </div>
                        </div> 
                        <div className="flex w-full justify-start items-center mt-3">
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-full mr-5">
                                <label>Is this case ended without any issue?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                                        checked={formRef.current.isAnyIssue }
                                        onChange={e => { formRef.current.isAnyIssue = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                            ...
                            <div className="w-full mr-5">
                                ...
                                <div className="flex ml-5">
                                    ...
                                    <input
                                        ...
                                        checked={formRef.current.isAnyFir }
                                        onChange={e => { formRef.current.isAnyFir = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                       ...
                       <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <div className="w-full mr-5">
                            <label>Case or FIR number</label>
                                    <input
                                        type="text"
                            ...
                            <div className="flex w-full justify-start items-center mt-3">
                             <div className="flex flex-col w-full">
                                <label>Comments</label>
                                    <textarea
                                        className={`w-full rounded border ${!formRef.current.policecaseComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        value={formRef.current.policecaseComments}
                                        placeholder="Type of Case: Domestic Violation , Financial crime , Sexual"
                                        onChange={e => { formRef.current.policecaseComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                        rows={4}
                                        >
                                    </textarea>
                            </div>
