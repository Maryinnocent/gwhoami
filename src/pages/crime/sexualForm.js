...
const SexualForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
        sexualAddedList.current[recordIndex].documents.push({
    ...
    const saveSexual = () => {
        ...
        : [
            {
              _modal: 'CrimeList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'sexual.id': arr.id },
              _data: { $set: { 'sexual.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Sexual Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Sexual Crime Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeSexual = () => {
      if (sexualAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Sexual Crime Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { sexual: { id: formRef.current.id } } },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this Sexual Crime Save Details?'
        );
      }
    };
    return (
            ...
            <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    formKey="isSexualCrime"
                    formRef={formRef}
                    ui={ui}
                    name="isSexualCrime"
                    label="Do you have any Sexual Crime?"
                    values={['Yes', 'No']}
                  ...
                  <InputRadio
                    ...
                    ui={ui}
                    name="isScPublic"
                    label="Is this in School / College or Public?"
                    values={['School', 'College','Public']}
                    required="School / College / Public  is required"
                  />
                  </div>
                  <div className="w-1/3 mr-5">
                  </div>
            ...
            <DatePicker
             ...
             <div className="w-1/3 mr-5">
                ...
                <div className="w-1/3 mr-5">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="isScaseFiled"
                    formRef={formRef}
                    ui={ui}
                    name="isScaseFiled"
                    label="Anyone filled the case?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                  </div>
                ...
                <div className="w-1/3 mr-5">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="anySJudgements"
                    formRef={formRef}
                    ui={ui}
                    name="anySJudgements"
                    label="Any Sexual Judgments?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                  </div>
