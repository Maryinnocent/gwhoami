...
const DrivingForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, drivingAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
        drivingAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveDriving = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'CrimeList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'driving': arr } } }] :
            [{ _modal: 'CrimeList', _condition: 'update', _find: { _id: pageData.current._id, 'driving.id': arr.id }, _data: { $set: { "driving.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Driving Deatails added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Driving and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeDriving = () => {
        if (drivingAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                ...
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Driving?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'CrimeList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'driving': { id: formRef.current.id } } } }];
            ...
            <div className="p-5 border rounded shadow-md relative" ref={currentDom}>
                ...
                <div className="pt-5 pb-3">
                            ...
                            <div className="w-1/3 mr-5">
                                ...
                                <div className="w-1/3 mr-5"> 
                                    <label>License Number</label>
                                            <input
                                                type="text"
                                                value={formRef.current.licenceNumber}
                                                className={`w-full rounded border ${!formRef.current.licenceNumber ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                                onChange={e => { formRef.current.licenceNumber = e.currentTarget.value; subRefresh(Date.now()); }}
                                            />
                                    </div>
                                </div>   
                    <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>Vin Number</label>
                                    <input
                                        type="text"
                                        value={formRef.current.vinNumber}
                                        className={`w-full rounded border ${!formRef.current.vinNumber ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.vinNumber = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                            </div>
                            <div className="w-1/3 mr-5">
                                <label>Make</label>
                                <select
                                    className={`border w-full p-2 rounded ${!formRef.current.make? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.make} onChange={e => { formRef.current.make = e.currentTarget.value; subRefresh(Date.now()) }}>
                                    <option value="">Select</option>
                                    {formList.make.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                </select>
                                </div>
                                <div className="w-1/3 mr-5"> 
                                    <label>Body Type</label>
                                    <select
                                        className={`border w-full p-2 rounded ${!formRef.current.bodyType? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.bodyType} onChange={e => { formRef.current.bodyType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">Select</option>
                                       {formList.bodyType.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                                </div>
                        </div>                
                    <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>Date</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.crimeDate ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.crimeDate ? new Date(formRef.current.crimeDate) : ''}
                                    onChange={date => { formRef.current.crimeDate = date; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                                <label>Milage</label>
                                <select
                                    className={`border w-full p-2 rounded ${!formRef.current.milage? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.milage} onChange={e => { formRef.current.milage = e.currentTarget.value; subRefresh(Date.now()) }}>
                                    <option value="">Select</option>
                                    {formList.milage.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                </select>
                                </div>
                                <div className="w-1/3 mr-5"> 
                                    <label>Fuel Type</label>
                                    <select
                                        className={`border w-full p-2 rounded ${!formRef.current.fuelType? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.fuelType} onChange={e => { formRef.current.fuelType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">Select</option>
                                       {formList.fuelType.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                                </div>
                        </div>  
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-full mr-5">
                                <label>Do you have any accident or violation in this year?</label>
                                <div className="flex ml-5">
                                <div class="mr-5">
                                    <input
                                        class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                                        checked={formRef.current.isRecentAccident}
                                        onChange={e => { formRef.current.isRecentAccident = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                          ...
                          <div  className={`w-full rounded ${false ? ' pointer-events-none': ''} ${!formRef.current.firstName ? 'invalidyear' : ''}`}>
                         {console.log('no')}
                         </div>
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>Do you have any International Accident?</label>
                                <div className="flex ml-5">
                                <div class="mr-5">
                                    <input
                                        ...
                                        checked={formRef.current.anyIntenationalAccident}
                                        onChange={e => { formRef.current.anyIntenationalAccident = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
