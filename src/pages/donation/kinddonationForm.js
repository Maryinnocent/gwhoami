...
const KinddonationForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      kinddonationAddedList.current[recordIndex].documents.push({
    ...
    const saveKinddonation = () => {
        ...
        : [
            {
              _modal: 'DonationList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'kinddonation.id': arr.id },
              _data: { $set: { 'kinddonation.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...kinddonationAddedList.current];
          newlist[recordIndex] = arr;
          kinddonationAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Kinddonation Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Kinddonation Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          kinddonationAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeKinddonation = () => {
      if (kinddonationAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            kinddonationAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Kinddonation Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { kinddonation: { id: formRef.current.id } } },
              },
            ];
          ...
          <div className="pt-5 pb-3">
            ...
            <div className="flex w-full justify-start items-center relative">
                ...
                <InputSelect
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Name In-Kind Item"
                    placeholder="--Select--"
                    options={formList.kindItem}
                    required="Name In-Kind Item is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <GroupInput
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="volunteering"
                    label="Are you doing any volunteering for kids Education?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
                ...
                <InputSelect
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Labour in weekly"
                    placeholder="--Select--"
                    options={formList.labour}
                    required="Labour in weekly is required"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.kindonatComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.kindonatComments}
...
export default KinddonationForm;
