...
const GiftsForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
      giftsAddedList.current[recordIndex].documents.push({
    ...
    const saveGifts = () => {
        ...
        : [
            {
              _modal: 'DonationList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'gifts.id': arr.id },
              _data: { $set: { 'gifts.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...giftsAddedList.current];
          newlist[recordIndex] = arr;
          giftsAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Gifts Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          giftsAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeGifts = () => {
      if (giftsAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            giftsAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Gifts Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { gifts: { id: formRef.current.id } } },
              },
            ];
...
<div className="flex w-full justify-start items-center relative">
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.giftComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.giftComments}
