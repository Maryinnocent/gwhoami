...
const ConstructionPanel = React.memo(
  ({ constructionAddedList, pageData, ui, uiRefresh }) => {
    const alertRef = useRef();
    const { scrollRef } = useContext(UserContext);
    const addConstruction = () => {
      let idx = constructionAddedList.current.findIndex(
      ...
      constructionAddedList.current.push({
        ...formList.construction,
        id: nanoid(),
      });
      uiRefresh(Date.now());
        ...
        <div className="flex w-full">
          <div className="w-3/4 justify-center">
            <h3 className="text-2xl">
              Construction of {MyLocalStorage.getLoginInfo().firstName}{' '}
