...
const OtherPanel = React.memo(({ otherAddedList, pageData, ui, uiRefresh }) => {
  ...
  const addOther = () => {
    let idx = otherAddedList.current.findIndex(
      (rec) => typeof rec.saved !== 'undefined'
    );
    if (idx !== -1) {
      ToastMessage({
        type: 'error',
        message: 'Please save Other Donation details!',
        timeout: 1000,
      });
      return;
    }
      ...
      <div className="flex w-full">
        <div className="w-3/4 justify-center">
          <h3 className="text-2xl">
            Other Donation Details of {MyLocalStorage.getLoginInfo().firstName}{' '}
        ...
        <div className="w-1/4 justify-end">
          <button
            ...
            onClick={addOther}
          >
            <i className="bx bx-plus mr-1 text-lg"></i> Add Other-Donation
          </button>
        </div>
      </div>
