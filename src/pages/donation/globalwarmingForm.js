...
const GlobalwarmingForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      globalwarmingAddedList.current[recordIndex].documents.push({
    ...
    const saveGlobalwarming = () => {
        ...
        : [
            {
              _modal: 'DonationList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'globalwarming.id': arr.id },
              _data: { $set: { 'globalwarming.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...globalwarmingAddedList.current];
          newlist[recordIndex] = arr;
          globalwarmingAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Globalwarming Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Certificate Save Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          globalwarmingAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeGlobalwarming = () => {
      if (globalwarmingAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            globalwarmingAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Globalwarming Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { globalwarming: { id: formRef.current.id } } },
              },
            ];
          ...
          <div className="pt-5 pb-3">
            ...
            <div className="flex w-full justify-start items-center relative">
                ...
                <InputSelect
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Name of Global Warming"
                    placeholder="--Select--"
                    options={formList.kindItem}
                    required="Name of Global Warming is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <GroupInput
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="isOrganizer"
                    label="Are you the one organized the Donation Drive?"
                    values={['Yes', 'No']}
                ...
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="isOnetime"
                    label="Is it onetime support or Monthly?"
                    values={['Onetime', 'Monthly']}
                    required="Onetime / Monthly  is required"
                  />
                </div>
              </div>
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="volunteering"
                    formRef={formRef}
                    ui={ui}
                    name="volunteering"
                    label="Are you doing any volunteering for Global Warming?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
                ...
                <InputSelect
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Support in weekly"
                    placeholder="--Select--"
                    options={formList.support}
                    required="Support in weekly is required"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.gwarmingComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.gwarmingComments}
                    placeholder="Descriptions/Activity:
                    Clothing and shoes , Toys and school supplies, Food
                    
                    Equipment or office supplies, Rent-free space, repair work and manual labour,Toys and games 
                    for kids, Clothing and shoes, Personal care items for care packages, First aid and medical supplies
                    
                    Fundraising activity also kind of in-kind Donation"
                    onChange={(e) => {
                      formRef.current.gwarmingComments =
...
export default GlobalwarmingForm;
