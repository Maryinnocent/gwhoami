...
const KidseducationForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      kidseducationAddedList.current[recordIndex].documents.push({
    ...
    const saveKidseducation = () => {
        ...
        : [
            {
              _modal: 'DonationList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'kidseducation.id': arr.id },
              _data: { $set: { 'kidseducation.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...kidseducationAddedList.current];
          newlist[recordIndex] = arr;
          kidseducationAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Kidseducation Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Kidseducation Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          kidseducationAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeKidseducation = () => {
      if (kidseducationAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            kidseducationAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Kidseducation Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { kidseducation: { id: formRef.current.id } } },
              },
            ];
          ...
          <div className="pt-5 pb-3">
            ...
            <div className="flex w-full justify-start items-center relative">
                ...
                <InputSelect
                    ...
                    formKey="donationType"
                    ID="donationType"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Donation Type"
                    placeholder="--Select--"
                    options={formList.donationType}
              ...
              <div className="flex w-full justify-start items-center relative">
                ...
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="isOnetime"
                    label="Is it onetime payment or Monthly?"
              ...
              <div className="flex w-full justify-start items-center relative">
                ...
                <InputSelect
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Hours in weekly"
                    placeholder="--Select--"
                    options={formList.kidsWeekly}
                    required="Hours in weekly is required"
                  />
                </div>
              </div>
...
export default KidseducationForm;
