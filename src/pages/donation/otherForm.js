...
const OtherForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
      otherAddedList.current[recordIndex].documents.push({
    ...
    const saveOther = () => {
        ...
        : [
            {
              _modal: 'DonationList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'other.id': arr.id },
              _data: { $set: { 'other.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...otherAddedList.current];
          newlist[recordIndex] = arr;
          otherAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Other Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Other Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          otherAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeOther = () => {
      if (otherAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            otherAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Other Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { other: { id: formRef.current.id } } },
              },
            ];
            ...
            <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    label="Organ Type"
                    placeholder="--Select--"
                    options={formList.organDonation}
