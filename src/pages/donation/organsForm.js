...
const OrgansForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
      organsAddedList.current[recordIndex].documents.push({
    ...
    const saveOrgans = () => {
        ...
        : [
            {
              _modal: 'DonationList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'organs.id': arr.id },
              _data: { $set: { 'organs.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...organsAddedList.current];
          newlist[recordIndex] = arr;
          organsAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Organs Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Organs Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          organsAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeOrgans = () => {
      if (organsAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            organsAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Organs Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { organs: { id: formRef.current.id } } },
              },
            ];
          ...
          <div className="pt-5 pb-3">
            <form>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    label="Organ Type"
                    placeholder="--Select--"
                    options={formList.organType}
                    required="Organ Type is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <GroupInput
              ...
              <div className="flex w-full justify-start items-center mt-3">
                ...
                <div className="w-1/3 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Age of Current $Year"
                    placeholder="Age of Current $Year"
                    required="Age of Current $Year is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <InputSelect
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="afterDeath"
                    label="Are you interested to donate your organ after death?"
                    values={['Yes', 'No']}
                ...
                <div className="w-1/3 mr-5">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="isFamilyaware"
                    formRef={formRef}
                    ui={ui}
                    name="isFamilyaware"
                    label="Is your family aware and agreed?"
                    values={['yes', 'No']}
                    required="Yes / No  is required"
                  />
                </div>
              ...
              <div className="flex w-full justify-start items-center relative">
                ...
                <DatePicker
                    styleClass="flex flex-col mb-4"
                    formKey="lastDonation"
                    ID="lastDonation"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Last time Donated"
                    placeholder="Last time Donated"
                    required="Last time Donated is required"
                    // callback={dobCallback}
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.organComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.organComments}
                    placeholder="DAFs,gifts of cash, gifts in a will
                    
                    real estate,Life insurance policies, Capital assets such as land, builsings, machinery, 
