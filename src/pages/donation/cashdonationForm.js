...
const CashdonationForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
        cashdonationAddedList.current[recordIndex].documents.push({
    ...
    const saveCashdonation = () => {
        ...
        : [
            {
              _modal: 'DonationList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'cashdonation.id': arr.id },
              _data: { $set: { 'cashdonation.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...cashdonationAddedList.current];
          newlist[recordIndex] = arr;
          cashdonationAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Cash Donations Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          cashdonationAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeCashdonation = () => {
      if (cashdonationAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            cashdonationAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Cashdonation Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { cashdonation: { id: formRef.current.id } } },
              },
            ];
          ...
          <div className="pt-5 pb-3">
            ...
            <div className="flex w-full justify-start items-center relative">
              ...
              <div className="flex w-full justify-start items-center mt-3">
                ...
                <DatePicker
                    styleClass="flex flex-col mb-4"
                    formKey="donationYear"
                    ID="donationYear"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Donation Year"
                    placeholder="Donation Year"
                    required="Donation Year is required"
