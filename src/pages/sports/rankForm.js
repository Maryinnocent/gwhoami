...
const RankForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, rankAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      rankAddedList.current[recordIndex].documents.push({
    ...
    const saveRank = () => {
        ...
        : [
            {
              _modal: 'SportList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'rank.id': arr.id },
              _data: { $set: { 'rank.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...rankAddedList.current];
          newlist[recordIndex] = arr;
          rankAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Rank Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Rank Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          rankAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeRank = () => {
      if (rankAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            rankAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Rank Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { rank: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.rankComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.rankComments}
                    onChange={(e) => {
                      formRef.current.rankComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
