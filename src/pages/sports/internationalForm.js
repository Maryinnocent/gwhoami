...
const InternationalForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      internationalAddedList.current[recordIndex].documents.push({
    ...
    const saveInternational = () => {
        ...
        : [
            {
              _modal: 'SportList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'international.id': arr.id },
              _data: { $set: { 'international.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...internationalAddedList.current];
          newlist[recordIndex] = arr;
          internationalAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'International Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the International Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          internationalAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeInternational = () => {
      if (internationalAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            internationalAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this International Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { international: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              <div className="flex w-full justify-start items-center relative">
                ...
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    label="Sports Type"
                    placeholder="--Select--"
                    options={formList.sportsType}
                    required="Sports Type is required"
                  />
                </div>
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    label="Level"
                    placeholder="--Select--"
                    options={formList.gameLevel}
                    required="Sports Level is required"
                  />
                </div>
              </div>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="px-2">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="isRecentINational"
                    formRef={formRef}
                    ui={ui}
                    name="isRecentINational"
                    label="Are You NationalWide Player?"
                    values={['yes', 'No']}
                ...
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    label="Club Name"
                    placeholder="--Select--"
                    options={formList.clubName}
                    required="Clubs Name is required"
                  />
                </div>
              </div>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.INationalComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.INationalComments}
                    onChange={(e) => {
                      formRef.current.INationalComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
