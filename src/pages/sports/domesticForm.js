...
const DomesticForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, domesticAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      domesticAddedList.current[recordIndex].documents.push({
    ...
    const saveDomestic = () => {
        ...
        : [
            {
              _modal: 'SportList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'domestic.id': arr.id },
              _data: { $set: { 'domestic.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...domesticAddedList.current];
          newlist[recordIndex] = arr;
          domesticAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Domestic Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Domestic Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          domesticAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeDomestic = () => {
      if (domesticAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            domesticAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Domestic Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { domestic: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="px-2">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="isRecentDomestic"
                    formRef={formRef}
                    ui={ui}
                    name="isRecentDomestic"
                    label="Is this your State Team?"
                    values={['yes', 'No']}
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex w-full mr-5">
                  <GroupInput
                    ...
                    required="Team Link is required"
                    icon={faSlash}
                  />
                </div>
              </div>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.domesticComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.domesticComments}
                    onChange={(e) => {
                      formRef.current.domesticComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
