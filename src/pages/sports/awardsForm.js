...
const AwardsForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, awardsAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      awardsAddedList.current[recordIndex].documents.push({
    ...
    const saveAwards = () => {
        ...
        : [
            {
              _modal: 'SportList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'awards.id': arr.id },
              _data: { $set: { 'awards.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...awardsAddedList.current];
          newlist[recordIndex] = arr;
          awardsAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Awards Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Awards Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          awardsAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeAwards = () => {
      if (awardsAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            awardsAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Awards Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { awards: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="isRecentAwards"
                    formRef={formRef}
                    ui={ui}
                    name="isRecentAwards"
                    label="Are you a Winner Team?"
                    values={['yes', 'No']}
                  ...
                  <div className="flex w-full justify-start items-center">
                    <div className="w-3/4 mr-2">
                      <InputSelect
                        ...
                        formRef={formRef}
                        uiRefresh={ui}
                        label="Participent Rank"
                        placeholder="--Select--"
                        options={formList.participent}
                        required="Participent Rank is required"
                      />
                    </div>
