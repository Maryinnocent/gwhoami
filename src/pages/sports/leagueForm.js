...
const LeagueForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, leagueAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      leagueAddedList.current[recordIndex].documents.push({
    ...
    const saveLeague = () => {
        ...
        : [
            {
              _modal: 'SportList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'league.id': arr.id },
              _data: { $set: { 'league.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...leagueAddedList.current];
          newlist[recordIndex] = arr;
          leagueAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'League Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the League Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          leagueAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeLeague = () => {
      if (leagueAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            leagueAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this League Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { league: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="px-2">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="isRecentLeague"
                    formRef={formRef}
                    ui={ui}
                    name="isRecentLeagues"
                    label="Is this your Primary Sports?"
                    values={['yes', 'No']}
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.leagueComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.leagueComments}
                    onChange={(e) => {
                      formRef.current.leagueComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
