...
const SpecificForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
      specificAddedList.current[recordIndex].documents.push({
    ...
    const saveSpecific = () => {
        ...
        : [
            {
              _modal: 'MarriageList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'specific.id': arr.id },
              _data: { $set: { 'specific.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...specificAddedList.current];
          newlist[recordIndex] = arr;
          specificAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Specification Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Specification Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          specificAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeSpecific = () => {
      if (specificAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            specificAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Specification Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { specific: { id: formRef.current.id } } },
              },
            ];
            ...
            <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    formKey="expectSpouce"
                    formRef={formRef}
                    ui={ui}
                    name="expectSpouce"
                    label="Do you have any specific expectations from spouse?"
                    values={['Yes', 'No']}
                ...
                <div className="w-1/3 mr-5">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="castSpecific"
                    formRef={formRef}
                    ui={ui}
                    name="castSpecific"
                    label="Do you have any Cast specification?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="jobProfile"
                    label="Are you looking for same Job profile?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
                ...
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="futurEdu"
                    label="Do you have any plan for future education?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="joinFamily"
                    label="Are you ok with joint-family?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
                ...
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.specificComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.specificComments}
                    placeholder="Description/Activity"
                    onChange={(e) => {
                      formRef.current.specificComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
