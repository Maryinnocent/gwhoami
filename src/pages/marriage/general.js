...
const GeneralPanel = React.memo(
  ({ generalAddedList, pageData, ui, uiRefresh }) => {
    const alertRef = useRef();
    const { scrollRef } = useContext(UserContext);
    const addGeneral = () => {
      let idx = generalAddedList.current.findIndex(
        (rec) => typeof rec.saved !== 'undefined'
      );
      if (idx !== -1) {
        ToastMessage({
          type: 'error',
          message: 'Please save General details!',
          timeout: 1000,
        });
        return;
      }
      generalAddedList.current.push({
        ...formList.general,
        id: nanoid(),
      });
      uiRefresh(Date.now());
      ...
      <div className="w-full">
        ...
        <div className="flex w-full">
          <div className="w-3/4 justify-center">
            ...
            General {MyLocalStorage.getLoginInfo().firstName}{' '}
          ...
          <div className="flex justify-end">
            <button
              ...
              onClick={addGeneral}
            >
              <i className="bx bx-plus mr-1 text-lg"></i> Add General
            </button>
          </div>
        </div>
