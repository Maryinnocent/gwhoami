...
const FutureplanForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      futureplanAddedList.current[recordIndex].documents.push({
    ...
    const saveFutureplan = () => {
        ...
        : [
            {
              _modal: 'MarriageList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'futureplan.id': arr.id },
              _data: { $set: { 'futureplan.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...futureplanAddedList.current];
          newlist[recordIndex] = arr;
          futureplanAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'futureplan Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the IRA Saving Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          futureplanAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeFutureplan = () => {
      if (futureplanAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            futureplanAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this IRA Saving Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { futureplan: { id: formRef.current.id } } },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this Futureplan Details?'
        );
      }
    };
    return (
            ...
            <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="Education"
                    label="Do have any any Education Plan?"
                    values={['Yes', 'No']}
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="kids"
                    label="Did you have any special plan for kids?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="spiritual"
                    label="Did you any spiritual plan?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
                ...
                <DatePicker
                    styleClass="flex flex-col mb-4"
                    formKey="spiritualPlan"
                    ID="spiritualPlan"
                    formRef={formRef}
                    uiRefresh={ui}
                    label="If any please"
                    placeholder="If any please"
                    required="If any please is required"
                    // callback={dobCallback}
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="financial"
                    label="Do you have any special financial planning?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="donation"
                    label="Do you have any Donation?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="volunteer"
                    label="Do you have any plan for volunteering?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
                ...
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.futureplanComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.futureplanComments}
                    placeholder="Description/Activity"
                    onChange={(e) => {
                      formRef.current.futureplanComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
