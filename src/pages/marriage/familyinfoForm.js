...
const FamilyinfoForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      familyinfoAddedList.current[recordIndex].documents.push({
    ...
    const saveFamilyinfo = () => {
        ...
        : [
            {
              _modal: 'MarriageList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'familyinfo.id': arr.id },
              _data: { $set: { 'familyinfo.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...familyinfoAddedList.current];
          newlist[recordIndex] = arr;
          familyinfoAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Familyinfo Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          familyinfoAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeFamilyinfo = () => {
      if (familyinfoAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            familyinfoAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Family Info Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { familyinfo: { id: formRef.current.id } } },
              },
            ];
            ...
            <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    ...
                    ui={ui}
                    name="withParents"
                    label="Are you staying with Parents?"
                    values={['Yes', 'No']}
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="grandparents"
                    formRef={formRef}
                    ui={ui}
                    name="grandparents"
                    label="Do you have grandparents?"
                    values={['Yes', 'No']}
                    required="Yes /No  is required"
                  />
                </div>
                ...
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.familyinfoComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.familyinfoComments}
