...
const BillingForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, billingAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      billingAddedList.current[recordIndex].documents.push({
    ...
    const saveBilling = () => {
        ...
        : [
            {
              _modal: 'BillingList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'billing.id': arr.id },
              _data: { $set: { 'billing.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...billingAddedList.current];
          newlist[recordIndex] = arr;
          billingAddedList.current = newlist;
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Billing Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          billingAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeBilling = () => {
      if (billingAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            billingAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Billing Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { billing: { id: formRef.current.id } } },
              },
            ];
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Bill Type [Grocery/Medical]"
                    placeholder="--Select--"
              ...
              <div className="flex w-full justify-start items-center mt-3">
                ...
                <div className="w-1/3 mr-5">
                  <DatePicker
                    ...
                    required="Bill Year is required"
                    // callback={dobCallback}
                  />
                </div>
                <div className="w-1/3 mr-5">
