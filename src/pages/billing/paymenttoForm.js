...
const PaymenttoForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      paymenttoAddedList.current[recordIndex].documents.push({
    ...
    const savePaymentto = () => {
        ...
        : [
            {
              _modal: 'BillingList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'paymentto.id': arr.id },
              _data: { $set: { 'paymentto.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...paymenttoAddedList.current];
          newlist[recordIndex] = arr;
          paymenttoAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Paymentto Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Paymentto Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          paymenttoAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removePaymentto = () => {
      if (paymenttoAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            paymenttoAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Paymentto Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { paymentto: { id: formRef.current.id } } },
              },
            ];
            ...
            <form>
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <InputSelect
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    label="Item Type [Grocery/Medical]"
                    placeholder="--Select--"
                    options={formList.billType}
              ...
              <div className="flex w-full justify-start items-center mt-3">
                <div className="w-1/3 mr-5">
                  <InputRadio
                    styleClass="flex flex-col mb-4"
                    formKey="isGwhoamiAccount"
                    formRef={formRef}
                    ui={ui}
                    name="isGwhoamiAccount"
                    label="Is this bill received by Email/Gwhoami account?"
                    values={['Yes', 'No']}
