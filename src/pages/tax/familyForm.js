...
const FamilyForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, familyAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      familyAddedList.current[recordIndex].documents.push({
    ...
    const saveFamily = () => {
        ...
        : [
            {
              _modal: 'TaxList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'family.id': arr.id },
              _data: { $set: { 'family.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...familyAddedList.current];
          newlist[recordIndex] = arr;
          familyAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'family Deatails added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the family and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          familyAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeFamily = () => {
      if (familyAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            familyAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
        ...
        alertRef.current.showConfirm(
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { family: { id: formRef.current.id } } },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this family details?'
        );
      }
    };
          ...
          <div className="pt-5 pb-3">
            <form>
              <div className="flex w-full justify-center  items-center relative">
                <h1>
                  Number of dependents you have as wellas any child care
                  expenses
                </h1>
              </div>
              <div className="flex w-full justify-start items-center relative">
                ...
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    uiRefresh={ui}
                    placeholder="0"
                    required="Enter Earned number of dependents"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Full time students age 17-23"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="unemploymentAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="0"
                    required="Enter Unemployment Income"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                ...
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    uiRefresh={ui}
                    placeholder="0"
                    required="Enter Number Of Under age 17"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Number of children under 13 who received child care"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    uiRefresh={ui}
                    placeholder="0"
                    required="Enter Number of children under 13"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="totalChildcare"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Total Child Care expenses"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    uiRefresh={ui}
                    placeholder="0"
                    required="Enter total child care expenses"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="How much childcare reimbursement was received in $year?"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="noReimbursement"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="0"
                    required="Enter child care reimbursement year"
                  />
                </div>
              </div>
                ...
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.familyComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.familyComments}
                    placeholder="Note : The value in 'Number of dependents' should include those listed in both 'Under age 17' and 'Full-time students age 17-23'"
                    onChange={(e) => {
                      formRef.current.familyComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
