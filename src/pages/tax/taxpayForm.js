...
const TaxpayForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, taxpayAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      taxpayAddedList.current[recordIndex].documents.push({
    ...
    const saveTaxpay = () => {
        ...
        : [
            {
              _modal: 'TaxList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'taxpay.id': arr.id },
              _data: { $set: { 'taxpay.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...taxpayAddedList.current];
          newlist[recordIndex] = arr;
          taxpayAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Taxpay Deatails added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Taxpay and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          taxpayAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeTaxpay = () => {
      if (taxpayAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            taxpayAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Taxpayments?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { taxpay: { id: formRef.current.id } } },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this Taxpayments details?'
        );
      }
    };
      ...
      <>
        ...
        <div className="p-5 border rounded shadow-md relative" ref={currentDom}>
          <i
            ...
            onClick={removeTaxpay}
          ></i>
          <div className="pt-5 pb-3">
            <form>
              <div className="flex w-full justify-center  items-center relative">
                <h1>
                  Enter any tax payments you have made directly to the IRS(not
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Federal taxes paid (employed paid, investment or Pension)"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="federalAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="$0.00"
                    required="Enter Federal taxes paid"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="State taxes paid (Income Tax, Sales Tax, Real Estate Tax and Property tax)"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="statetaxAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="$0.00"
                    required="Enter State taxes paid"
                  />
                </div>
              </div>
                ...
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.taxpayComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.taxpayComments}
                    placeholder="w-2 income.  
                    Interest or dividends(1099-INT/1099-DIV) that don't require filling a schedule 8 .
                    IRS standard deduction .
                    Child Tax Credit(CTC) .  
                    Student Loan interest deduction"
                    onChange={(e) => {
                      formRef.current.taxpayComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
