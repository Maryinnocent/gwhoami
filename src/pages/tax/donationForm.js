...
const DonationForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, donationAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      donationAddedList.current[recordIndex].documents.push({
    ...
    const saveDonation = () => {
        ...
        : [
            {
              _modal: 'TaxList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'donation.id': arr.id },
              _data: { $set: { 'donation.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...donationAddedList.current];
          newlist[recordIndex] = arr;
          donationAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Donation Deatails added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the donation details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          donationAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeDonation = () => {
      if (donationAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            donationAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
        ...
        alertRef.current.showConfirm(
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { donation: { id: formRef.current.id } } },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this donation details?'
        );
      }
    };
          ...
          <div className="pt-5 pb-3">
            <form>
              <div className="flex w-full justify-center  items-center relative">
                <h1>
                  Enter the annual amount of monetary donations you make to
                  charity,as well as the fair-market value of non-cash donations
                  like clothes, furniture etc...:-
                </h1>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formKey="cashDonation"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Check or Cash Donation"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="donationAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="$0.00"
                    required="Enter Check or Cash Donation"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Total non-cash donations"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    uiRefresh={ui}
                    placeholder="$0.00"
                    required="Enter total non-cash donations"
                  />
                </div>
              </div>
