...
const OincomeForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, oincomeAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      oincomeAddedList.current[recordIndex].documents.push({
    ...
    const saveOincome = () => {
        ...
        : [
            {
              _modal: 'TaxList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'oincome.id': arr.id },
              _data: { $set: { 'oincome.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...oincomeAddedList.current];
          newlist[recordIndex] = arr;
          oincomeAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Other Income Deatails added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Other Income and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          oincomeAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeOincome = () => {
      if (oincomeAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            oincomeAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this Other Income?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { oincome: { id: formRef.current.id } } },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this oincome details?'
        );
      }
    };
      ...
      <>
        ...
        <div className="p-5 border rounded shadow-md relative" ref={currentDom}>
          <i
            ...
            onClick={removeOincome}
          ></i>
          <div className="pt-5 pb-3">
            <form>
              <div className="flex w-full justify-center  items-center relative">
                <h1>
                  Enter Other income like investments, Social Security, or
                  Pensions
                </h1>
              </div>
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Gains/Losses(short-term)"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    uiRefresh={ui}
                    placeholder="$0"
                    required="Enter Gains/Losses(short-term)"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="IRA/Pension distributions"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    uiRefresh={ui}
                    placeholder="$0"
                    required="Enter IRA/Pension distributions"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
              ...
              <div className="flex w-full justify-start items-center relative">
                ...
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="dividendAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="$0"
                    required="Enter Qualified Dividends"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Gain/Losses (Long-term)"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="gainLongAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="$0"
                    required="Enter Gain/Losses (Long-term)"
                  />
                </div>
              </div>
                ...
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.oincomeComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.oincomeComments}
                    placeholder="Description / Activity"
                    onChange={(e) => {
                      formRef.current.oincomeComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
