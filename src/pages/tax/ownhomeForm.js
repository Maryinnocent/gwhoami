...
const ownhomeForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, homeAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      homeAddedList.current[recordIndex].documents.push({
    ...
    const saveHome = () => {
        ...
        : [
            {
              _modal: 'TaxList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'home.id': arr.id },
              _data: { $set: { 'home.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...homeAddedList.current];
          newlist[recordIndex] = arr;
          homeAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'Home Deatails added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Home and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          homeAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeHome = () => {
      if (homeAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            homeAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
        ...
        alertRef.current.showConfirm(
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { home: { id: formRef.current.id } } },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this Home details?'
        );
      }
    };
          ...
          <div className="pt-5 pb-3">
            <form>
              <div className="flex w-full justify-center  items-center relative">
                <h1>
                  Enter you and your spouse home Mortgage and property tax{' '}
                </h1>
              </div>
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Mortgage interest paid(Form 1098)"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="mortgageAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="$0.00"
                    required="Enter mortgage interest paid"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                ...
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    uiRefresh={ui}
                    placeholder="$0.00"
                    required="Enter Propert taxes paid"
                  />
                </div>
              </div>
