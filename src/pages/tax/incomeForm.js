...
const IncomeForm = React.memo(
  ({ form, uiRefresh, alertRef, pageData, recordIndex, incomeAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
      incomeAddedList.current[recordIndex].documents.push({
    ...
    const saveIncome = () => {
        ...
        : [
            {
              _modal: 'TaxList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'income.id': arr.id },
              _data: { $set: { 'income.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Income Deatails added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the income and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeIncome = () => {
      if (incomeAddedList.current[recordIndex].saved === false) {
        ...
        alertRef.current.showConfirm(
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { income: { id: formRef.current.id } } },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this income details?'
        );
      }
    };
          ...
          <div className="pt-5 pb-3">
            <form>
              <div className="flex w-full justify-center  items-center relative">
                <h1>Enter your annual earned income</h1>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Earned Money jobs $year"
                    required="With held federal taxes $year"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="earnedAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="$0.00"
                    required="Enter Earned Money"
                  />
                </div>
              </div>
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="unemploymentIncome "
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Unemployment income $year"
                    required="Unemployment income $year"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="federalTaxes"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="With held federal taxes $year"
                    required="With held federal taxes is required"
                    icon={faUser}
                  />
                </div>
                <div className="w-1/2 mr-5">
                ...
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.incomeComments
                    ...
                    onChange={(e) => {
                      formRef.current.incomeComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
