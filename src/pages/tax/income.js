import { nanoid } from 'nanoid';
import React, { useRef } from 'react';
import { useContext } from 'react';
import AlertDialog from '../../helper/alertDialog';
import ToastMessage from '../../toast';
import { UserContext } from '../../util/maincontext';
import { formList } from './formLists';
import IncomeForm from './incomeForm';
import MyLocalStorage from '../../util/mylocalStorage';

const IncomePanel = React.memo(
  ({ incomeAddedList, pageData, ui, uiRefresh }) => {
    const alertRef = useRef();
    const { scrollRef } = useContext(UserContext);
    const addIncome = () => {
      let idx = incomeAddedList.current.findIndex(
        (rec) => typeof rec.saved !== 'undefined'
      );
      if (idx !== -1) {
        ToastMessage({
          type: 'error',
          message: 'Please save Income member details!',
          timeout: 1000,
        });
        return;
      }
      incomeAddedList.current.push({ ...formList.income, id: nanoid() });
      uiRefresh(Date.now());
      setTimeout(() => scrollRef.current.scrollToBottom(), 200);
    };

    return (
      <div className="w-full">
        <AlertDialog ref={alertRef} title={'Confirm to Delete?'} />
        <div className="flex w-full">
          <div className="w-3/4 justify-center">
            <h3 className="text-2xl">
              Income Details of {MyLocalStorage.getLoginInfo().firstName}{' '}
              {MyLocalStorage.getLoginInfo().lastName}
            </h3>
          </div>
          <div className="w-1/4 justify-end">
            <button
              className="bg-dodge-d px-3 py-1.5 text-white text-sm shadow-md flex items-end hover:bg-dodge-b"
              onClick={addIncome}
            >
              <i className="bx bx-plus mr-1 text-lg"></i> Add Income details
            </button>
          </div>
        </div>
        {incomeAddedList.current.map((item, idx) => (
          <div className="mt-5" key={item.id}>
            <IncomeForm
              form={item}
              ui={ui}
              uiRefresh={uiRefresh}
              alertRef={alertRef}
              pageData={pageData}
              recordIndex={idx}
              incomeAddedList={incomeAddedList}
            />
          </div>
        ))}
      </div>
    );
  }
);

export default IncomePanel;
