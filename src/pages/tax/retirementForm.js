...
const RetirementForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      retirementAddedList.current[recordIndex].documents.push({
    ...
    const saveRetirement = () => {
        ...
        : [
            {
              _modal: 'TaxList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'retirement.id': arr.id },
              _data: { $set: { 'retirement.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          arr.isSubmit = true;
          let newlist = [...retirementAddedList.current];
          newlist[recordIndex] = arr;
          retirementAddedList.current = newlist;
          pageRef.current.isSaving = false;
          formRef.current = { ...arr };
          uiRefresh(Date.now());
          ToastMessage({
            type: 'success',
            message: 'retirement Deatails added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the retirement and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeFile = (itm, idx) => {
      alertRef.current.showConfirm(
        (res) => {
          if (res === 'no') return;
          retirementAddedList.current[recordIndex].documents.splice(idx, 1);
          subRefresh(Date.now());
    ...
    const removeRetirement = () => {
      if (retirementAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          (res) => {
            if (res === 'no') return;
            retirementAddedList.current.splice(recordIndex, 1);
            uiRefresh(Date.now());
          },
          'Confirm?',
          'Are you sure to delete this retirement?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { retirement: { id: formRef.current.id } } },
              },
            ];
          ...
          'Confirm?',
          'Are you sure to delete this retirement details?'
        );
      }
    };
      ...
      <>
        ...
        <div className="p-5 border rounded shadow-md relative" ref={currentDom}>
          <i
            ...
            onClick={removeRetirement}
          ></i>
          <div className="pt-5 pb-3">
            <form>
              <div className="flex w-full justify-center  items-center relative">
                <h1>
                  If you "(or your spouse)" contributed to a retirement plan or
                  an IRA for $year, provide details below.
                </h1>
              </div>
              <div className="flex w-full justify-center items-center relative">
                <div className="px-2">
                  <InputRadio
                    ...
                    ui={ui}
                    name="retairPlan"
                    label="Are you eligible to participate in a retirement plan at work?"
                    values={['yes', 'No']}
              ...
              <div className="flex w-full justify-start items-center relative">
                <div className="w-1/2 mr-5">
                  <GroupInput
                    ...
                    formKey="iraContribution"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="Traditional IRA Contributions (not 401(k))"
                  />
                </div>
                <div className="w-1/2 mr-5">
                  <GroupInput
                    styleClass="flex flex-col mb-4"
                    formKey="iraRetairAmount"
                    formRef={formRef}
                    uiRefresh={ui}
                    placeholder="$0.00"
                    required="Enter Traditional IRA Contributions"
                ...
                <div className="flex flex-col w-full">
                  ...
                  <textarea
                    className={`w-full rounded border ${
                      !formRef.current.retirementComments
                        ? 'border-red-500 err-input'
                        : 'border-gray-400'
                    }`}
                    value={formRef.current.retirementComments}
                    placeholder="Description / Activity"
                    onChange={(e) => {
                      formRef.current.retirementComments = e.currentTarget.value;
                      subRefresh(Date.now());
                    }}
                    rows={4}
                  ></textarea>
