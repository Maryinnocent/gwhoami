...
const ResearchForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, researchAddedList }) => {
    ...
    const completeHandler = (event) => {
        researchAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
   ...
   const countryCallback = (code, itm, idx) => {
    ...
    const saveResearch = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'MemberofList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'research': arr } } }] :
            [{ _modal: 'MemberofList', _condition: 'update', _find: { _id: pageData.current._id, 'research.id': arr.id }, _data: { $set: { "research.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...researchAddedList.current];
                newlist[recordIndex] = arr;
                researchAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Research added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Research and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            researchAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeResearch = () => {
        if (researchAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                researchAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Research?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'MemberofList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'research': { id: formRef.current.id } } } }];
                ...
                <div className="pt-5 pb-3">
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5"> 
                                    ...
                                    <select
                                        className={`border w-full p-2 rounded ${!formRef.current.researchGroupName? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.researchGroupName} onChange={e => { formRef.current.researchGroupName = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.researchGroupName.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                                </div>
                            <div className="w-1/3 mr-5">
                                <label>Group Type</label>
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.researchGroupType? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.researchGroupType} onChange={e => { formRef.current.researchGroupType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.researchGroupType.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                            </div>
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                        type="text"
                                        value={formRef.current.researchOrgName}
                                        placeholder="Organization Name"
                                        className={`w-full rounded border ${!formRef.current.researchOrgName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.researchOrgName = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                            </div>   
                         ...
                         <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <label>Group Since</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.researchGroupSince ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.researchGroupSince ? new Date(formRef.current.researchGroupSince) : ''}
                                    onChange={date => { formRef.current.researchGroupSince = date; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>Member of Since</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.researchMemberSince? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.researchMemberSince ? new Date(formRef.current.researchMemberSince) : ''}
                                    onChange={date => { formRef.current.researchMemberSince = date; subRefresh(Date.now()); }}
                                />
                                </div>
                                <div className="w-1/3 mr-5"> 
                                    <label>Number of Members</label>
                                            <input
                                                type="text"
                                                value={formRef.current.researchNumberMember}
                                                placeholder="Number of Members"
                                                className={`w-full rounded border ${!formRef.current.researchNumberMember ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                                onChange={e => { formRef.current.researchNumberMember = e.currentTarget.value; subRefresh(Date.now()); }}
                                            />
                                </div>
                        </div> 
                        <div className="flex w-full justify-start items-center mt-3">
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>Is this Education/ Science /Space?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                                        checked={formRef.current.isResearch }
                                        onChange={e => { formRef.current.isResearch = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                            ...
                            <label>Are you a member of Board?</label>
                                <div className="flex ml-5">
                                    ...
                                    <input
                                        ...
                                        checked={formRef.current.isRdBoardMember }
                                        onChange={e => { formRef.current.isRdBoardMember = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5"> 
                            <label>Have you publish any New Research?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        ...
                                        checked={formRef.current.anyResearch }
                                        onChange={e => { formRef.current.anyResearch = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                            ...
                            <div className="w-1/3 mr-5">
                                ...
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.researchPosition? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.researchPosition} onChange={e => { formRef.current.researchPosition = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.researchPosition.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                            </div>
                            <div className="w-1/3 mr-5">
                            ...
                            <input
                                        type="text"
                                        value={formRef.current.researchNumYears}
                                        placeholder="Number Of Years"
                                        className={`w-full rounded border ${!formRef.current.researchNumYears ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.researchNumYears = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                            </div>   
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                <label>Comments</label>
                                    <textarea
                                        className={`w-full rounded border ${!formRef.current.vComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        value={formRef.current.researchComments}
                                        placeholder="Description / Activity"
                                        onChange={e => { formRef.current.researchComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                        rows={4}
                                        >
                                    </textarea>
                            </div>
