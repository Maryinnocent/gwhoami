...
const ResearchForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
      researchAddedList.current[recordIndex].documents.push({
    ...
    const saveResearch = () => {
        ...
        : [
            {
              _modal: 'MemberofList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'research.id': arr.id },
              _data: { $set: { 'research.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Research Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Research Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeResearch = () => {
      if (researchAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Research Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { research: { id: formRef.current.id } } },
              },
            ];
