...
const SecurityForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
      securityAddedList.current[recordIndex].documents.push({
    ...
    const saveSecurity = () => {
        ...
        : [
            {
              _modal: 'MemberofList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'security.id': arr.id },
              _data: { $set: { 'security.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Security Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Security Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeSecurity = () => {
      if (securityAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Security Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { security: { id: formRef.current.id } } },
              },
            ];
