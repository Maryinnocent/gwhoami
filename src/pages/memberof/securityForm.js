...
const SecurityForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, securityAddedList }) => {
    ...
    const completeHandler = (event) => {
        securityAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
   ...
   const countryCallback = (code, itm, idx) => {
    ...
    const saveSecurity = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'MemberofList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'security': arr } } }] :
            [{ _modal: 'MemberofList', _condition: 'update', _find: { _id: pageData.current._id, 'security.id': arr.id }, _data: { $set: { "security.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...securityAddedList.current];
                newlist[recordIndex] = arr;
                securityAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Security added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Security and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            securityAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeSecurity = () => {
        if (securityAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                securityAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Security?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'MemberofList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'security': { id: formRef.current.id } } } }];
                ...
                <div className="pt-5 pb-3">
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5"> 
                                    ...
                                    <select
                                        className={`border w-full p-2 rounded ${!formRef.current.securGroupName? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.securGroupName} onChange={e => { formRef.current.securGroupName = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.securGroupName.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                                </div>
                            <div className="w-1/3 mr-5">
                                <label>Group Type</label>
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.securGroupType? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.securGroupType} onChange={e => { formRef.current.securGroupType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.securGroupType.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                            </div>
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                        type="text"
                                        value={formRef.current.securOrgName}
                                        placeholder="Organization Name"
                                        className={`w-full rounded border ${!formRef.current.securOrgName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.securOrgName = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                            </div>   
                         ...
                         <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <label>Group Since</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.securGroupSince ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.securGroupSince ? new Date(formRef.current.securGroupSince) : ''}
                                    onChange={date => { formRef.current.securGroupSince = date; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>Member of Since</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.securMemberSince? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.securMemberSince ? new Date(formRef.current.securMemberSince) : ''}
                                    onChange={date => { formRef.current.securMemberSince = date; subRefresh(Date.now()); }}
                                />
                                </div>
                                <div className="w-1/3 mr-5"> 
                                    <label>Number of Members</label>
                                            <input
                                                type="text"
                                                value={formRef.current.securNumberMember}
                                                placeholder="Number of Members"
                                                className={`w-full rounded border ${!formRef.current.securNumberMember ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                                onChange={e => { formRef.current.securNumberMember = e.currentTarget.value; subRefresh(Date.now()); }}
                                            />
                                </div>
                        </div> 
                        <div className="flex w-full justify-start items-center mt-3">
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>Is this IT/Cyber Security?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                                        checked={formRef.current.isSecurity }
                                        onChange={e => { formRef.current.isSecurity = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                            ...
                            <label>Are you a member of Board?</label>
                                <div className="flex ml-5">
                                    ...
                                    <input
                                        ...
                                        checked={formRef.current.isSecurBoardMember }
                                        onChange={e => { formRef.current.isSecurBoardMember = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <div className="w-1/3 mr-5">
                                ...
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.securPosition? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.securPosition} onChange={e => { formRef.current.securPosition = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.securPosition.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                            </div>
                            <div className="w-1/3 mr-5">
                            ...
                            <input
                                        type="text"
                                        value={formRef.current.securNumYears}
                                        placeholder="Number Of Years"
                                        className={`w-full rounded border ${!formRef.current.securNumYears ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.securNumYears = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                            </div>   
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                <label>Comments</label>
                                    <textarea
                                        className={`w-full rounded border ${!formRef.current.securComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        value={formRef.current.securComments}
                                        placeholder="Description / Activity"
                                        onChange={e => { formRef.current.securComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                        rows={4}
                                        >
                                    </textarea>
                            </div>
