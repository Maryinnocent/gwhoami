...
const VolunteerForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
      volunteerAddedList.current[recordIndex].documents.push({
    ...
    const saveVolunteer = () => {
        ...
        : [
            {
              _modal: 'MemberofList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'volunteer.id': arr.id },
              _data: { $set: { 'volunteer.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Volunteer Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Volunteer Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeVolunteer = () => {
      if (volunteerAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Volunteer Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { volunteer: { id: formRef.current.id } } },
              },
            ];
