...
const VolunteerForm = React.memo(({ form, uiRefresh, alertRef, pageData, recordIndex, volunteerAddedList }) => {
    const [ui] = useState(-1);
    ...
    const completeHandler = (event) => {
        volunteerAddedList.current[recordIndex].documents.push({ ...pageRef.current.file_record });
    ...
    const saveVolunteer = () => {
        ...
        delete arr['isSubmit'];
        let params = isNew ? [{ _modal: 'MemberofList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $push: { 'volunteer': arr } } }] :
            [{ _modal: 'MemberofList', _condition: 'update', _find: { _id: pageData.current._id, 'volunteer.id': arr.id }, _data: { $set: { "volunteer.$": arr } }, _options: { upsert: false } }];
        (async () => {
            ...
            if (res.isError) {
                ...
                arr.isSubmit = true;
                let newlist = [...volunteerAddedList.current];
                newlist[recordIndex] = arr;
                volunteerAddedList.current = newlist;
                pageRef.current.isSaving = false;
                formRef.current = { ...arr }
                uiRefresh(Date.now());
                ToastMessage({ type: 'success', message: 'Volunteer Deatails added succesfully!', timeout: 1200 });
            }
        })();
    }
    const openFileUpload = () => {
        if (typeof formRef.current.saved !== 'undefined') {
            ToastMessage({ type: 'error', message: 'Save the Volunteer and upload!', timeout: 1200 });
            return;
        }
    ...
    const removeFile = (itm, idx) => {
        alertRef.current.showConfirm((res) => {
            if (res === 'no') return;
            volunteerAddedList.current[recordIndex].documents.splice(idx, 1);
            subRefresh(Date.now());
    ...
    const removeVolunteer = () => {
        if (volunteerAddedList.current[recordIndex].saved === false) {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                volunteerAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Volunteer?');
        } else {
            alertRef.current.showConfirm((res) => {
                if (res === 'no') return;
                let params = [{ _modal: 'MemberofList', _condition: 'update', _find: { _id: pageData.current._id }, _data: { $pull: { 'volunteer': { id: formRef.current.id } } } }];
                apiPostCall('/api/common/common_mutiple_insert', { _list: params });
                volunteerAddedList.current.splice(recordIndex, 1);
                uiRefresh(Date.now());
            }, 'Confirm?', 'Are you sure to delete this Volunteer details?');
        }
    }
                ...
                <div className="pt-5 pb-3">
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5"> 
                                    ...
                                    <select
                                        className={`border w-full p-2 rounded ${!formRef.current.volunGroupName? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.volunGroupName} onChange={e => { formRef.current.volunGroupName = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.volunGroupName.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                                </div>
                            <div className="w-1/3 mr-5">
                                <label>Group Type</label>
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.volunGroupType? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.volunGroupType} onChange={e => { formRef.current.volunGroupType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.volunGroupType.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                            </div>
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                        type="text"
                                        value={formRef.current.volunOrgName}
                                        placeholder="Organization Name"
                                        className={`w-full rounded border ${!formRef.current.volunOrgName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.volunOrgName = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                            </div>   
                         ...
                         <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <label>Group Since</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.volunGroupSince ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.volunGroupSince ? new Date(formRef.current.volunGroupSince) : ''}
                                    onChange={date => { formRef.current.volunGroupSince = date; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>Member of Since</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.volunMemberSince? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.volunMemberSince ? new Date(formRef.current.volunMemberSince) : ''}
                                    onChange={date => { formRef.current.volunMemberSince = date; subRefresh(Date.now()); }}
                                />
                                </div>
                                <div className="w-1/3 mr-5"> 
                                    <label>Number of Members</label>
                                            <input
                                                type="text"
                                                value={formRef.current.volunNumberMember}
                                                placeholder="Number of Members"
                                                className={`w-full rounded border ${!formRef.current.volunNumberMember ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                                onChange={e => { formRef.current.volunNumberMember = e.currentTarget.value; subRefresh(Date.now()); }}
                                            />
                                </div>
                        </div> 
                        <div className="flex w-full justify-start items-center mt-3">
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>Is this group being Nation wide?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                                        checked={formRef.current.isVolunteer }
                                        onChange={e => { formRef.current.isVolunteer = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                            ...
                            <label>Are you a member of Board?</label>
                                <div className="flex ml-5">
                                    ...
                                    <input
                                        ...
                                        checked={formRef.current.isVolunBoardMember }
                                        onChange={e => { formRef.current.isVolunBoardMember = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <div className="w-1/3 mr-5">
                                ...
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.volunPosition? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.volunPosition} onChange={e => { formRef.current.volunPosition = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.volunPosition.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                            </div>
                            <div className="w-1/3 mr-5">
                            ...
                            <input
                                        type="text"
                                        value={formRef.current.volunNumYears}
                                        placeholder="Number Of Years"
                                        className={`w-full rounded border ${!formRef.current.volunNumYears ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.volunNumYears = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                            </div>   
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                <label>Comments</label>
                                    <textarea
                                        className={`w-full rounded border ${!formRef.current.volunComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        value={formRef.current.volunComments}
                                        placeholder="Description / Activity"
                                        onChange={e => { formRef.current.volunComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                        rows={4}
                                        >
                                    </textarea>
                            </div>
