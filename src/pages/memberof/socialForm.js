...
const SocialForm = React.memo(
  ({
    ...
    const completeHandler = (event) => {
      socialAddedList.current[recordIndex].documents.push({
    ...
    const saveSocial = () => {
        ...
        : [
            {
              _modal: 'MemberofList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'social.id': arr.id },
              _data: { $set: { 'social.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Social Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Social Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeSocial = () => {
      if (socialAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Social Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { social: { id: formRef.current.id } } },
              },
            ];
