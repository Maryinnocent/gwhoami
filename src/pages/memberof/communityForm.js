...
const CommunityForm = React.memo(
  ({
    form,
    uiRefresh,
    alertRef,
    ...
    const completeHandler = (event) => {
        communityAddedList.current[recordIndex].documents.push({
    ...
    const saveCommunity = () => {
        ...
        : [
            {
              _modal: 'MemberofList',
              _condition: 'update',
              _find: { _id: pageData.current._id, 'community.id': arr.id },
              _data: { $set: { 'community.$': arr } },
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Community Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Community Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeCommunity = () => {
      if (communityAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Community Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
            ...
            let params = [
              {
                ...
                _condition: 'update',
                _find: { _id: pageData.current._id },
                _data: { $pull: { community: { id: formRef.current.id } } },
              },
            ];
