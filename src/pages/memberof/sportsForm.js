                ...
                <div className="pt-5 pb-3">
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5"> 
                                    ...
                                    <select
                                        className={`border w-full p-2 rounded ${!formRef.current.sportGroupName? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.sportGroupName} onChange={e => { formRef.current.sportGroupName = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.sportGroupName.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                                </div>
                            <div className="w-1/3 mr-5">
                                <label>Group Type</label>
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.sportGroupType? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.sportGroupType} onChange={e => { formRef.current.sportGroupType = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.sportGroupType.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                            </div>
                            <div className="w-1/3 mr-5">
                                ...
                                <input
                                        type="text"
                                        value={formRef.current.sportOrgName}
                                        placeholder="Organization Name"
                                        className={`w-full rounded border ${!formRef.current.sportOrgName ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.sportOrgName = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                            </div>   
                         ...
                         <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <label>Group Since</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.sportGroupSince ? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.sportGroupSince ? new Date(formRef.current.sportGroupSince) : ''}
                                    onChange={date => { formRef.current.sportGroupSince = date; subRefresh(Date.now()); }}
                                />
                            </div>
                            <div className="w-1/3 mr-5">
                            <label>Member of Since</label>
                                <Datetime
                                    className={`w-full rounded ${!formRef.current.sportMemberSince? 'invalidyear' : ''}`}
                                    placeholder="MM/DD/YYYY"
                                    dateFormat="MM/DD/YYYY"
                                    closeOnSelect={true}
                                    timeFormat={false}
                                    inputProps={inputProps}
                                    value={formRef.current.sportMemberSince ? new Date(formRef.current.sportMemberSince) : ''}
                                    onChange={date => { formRef.current.sportMemberSince = date; subRefresh(Date.now()); }}
                                />
                                </div>
                                <div className="w-1/3 mr-5"> 
                                    <label>Number of Members</label>
                                            <input
                                                type="text"
                                                value={formRef.current.sportNumberMember}
                                                placeholder="Number of Members"
                                                className={`w-full rounded border ${!formRef.current.sportNumberMember ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                                onChange={e => { formRef.current.sportNumberMember = e.currentTarget.value; subRefresh(Date.now()); }}
                                            />
                                </div>
                        </div> 
                        <div className="flex w-full justify-start items-center mt-3">
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="w-1/3 mr-5">
                                <label>are you playing or your kids playing?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        class="form-check-input appearance-none rounded-full h-4 w-4 border border-gray-300 bg-white checked:bg-blue-600 checked:border-blue-600 focus:outline-none transition duration-200 mt-1 align-top bg-no-repeat bg-center bg-contain float-left mr-2 cursor-pointer" type="radio" name="flexRadioDefault" id="flexRadioDefault1"
                                        checked={formRef.current.isSports }
                                        onChange={e => { formRef.current.isSports = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                            ...
                            <div className="w-1/3 mr-5">
                            <label>Are you a member of Sports Board?</label>
                                <div className="flex ml-5">
                                    <div class="mr-5">
                                    <input
                                        ...
                                        checked={formRef.current.isSportBoardMember }
                                        onChange={e => { formRef.current.isSportBoardMember = e.currentTarget.checked; subRefresh(Date.now()); }}
                                    />
                    ...
                    <div className="flex w-full justify-start items-center mt-3">
                            ...
                            <div className="w-1/3 mr-5">
                                ...
                                <select
                                        className={`border w-full p-2 rounded ${!formRef.current.sportPosition? 'border-red-500 err-input' : 'border-gray-400'}`} defaultValue={formRef.current.sportPosition} onChange={e => { formRef.current.sportPosition = e.currentTarget.value; subRefresh(Date.now()) }}>
                                       <option value="">--Select--</option>
                                       {formList.sportPosition.map((itm, idx) => <option key={idx} value={itm.key || itm}>{itm.name || itm}</option>)}
                                    </select>
                            </div>
                            <div className="w-1/3 mr-5">
                            ...
                            <input
                                        type="text"
                                        value={formRef.current.sportNumYears}
                                        placeholder="Number Of Years"
                                        className={`w-full rounded border ${!formRef.current.sportNumYears ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        onChange={e => { formRef.current.sportNumYears = e.currentTarget.value; subRefresh(Date.now()); }}
                                    />
                                </div>
                            </div>   
                        ...
                        <div className="flex w-full justify-start items-center mt-3">
                            <div className="flex flex-col w-full">
                                <label>Comments</label>
                                    <textarea
                                        className={`w-full rounded border ${!formRef.current.sportComments ? 'border-red-500 err-input' : 'border-gray-400'}`}
                                        value={formRef.current.sportComments}
                                        placeholder="Description / Activity"
                                        onChange={e => { formRef.current.sportComments = e.currentTarget.value; subRefresh(Date.now()); }}
                                        rows={4}
                                        >
                                    </textarea>
                            </div>
