  ...
  ({
    ...
    const saveSports = () => {
      ...
      (async () => {
        ...
        if (res.isError) {
          ...
          ToastMessage({
            type: 'success',
            message: 'Sports Details added succesfully!',
            timeout: 1200,
          });
        }
      })();
    ...
    const openFileUpload = () => {
      if (typeof formRef.current.saved !== 'undefined') {
        ToastMessage({
          type: 'error',
          message: 'Save the Sports Details and upload!',
          timeout: 1200,
        });
        return;
      }
    ...
    const removeSports = () => {
      if (sportsAddedList.current[recordIndex].saved === false) {
        alertRef.current.showConfirm(
          ...
          'Confirm?',
          'Are you sure to delete this Sports Details?'
        );
      } else {
        alertRef.current.showConfirm(
          (res) => {
